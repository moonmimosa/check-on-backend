...
async def report(body: ReportBody, req: Request):
    ...
    if body.event == "open":
        cur = conn.cursor()
        cur.execute(
            "SELECT id, app_name FROM records "
...
def _get_summary():
    ...
    for r in rows:
        ...
        if ev == "open":
            ...
            sessions[app] = sessions.get(app, 0) + int(
                (datetime.fromisoformat(ts) - opens[app]).total_seconds()
            )
            del opens[app]

    # 关键修复：正在使用的 App，用当前时间算实时时长
...
TOOLS = [
    {
        "name": "check_on_wife",
        "description": "查岗手机活动，获取最近使用的App和使用时长",
        "inputSchema": {"type": "object", "properties": {"limit": {"type": "integer"}}},
    },
    {
        "name": "bark_alert",
        # 🔥 只改了这里：描述改通用
        "description": "给手机发Bark推送弹窗（通用工具，不限于查岗场景，title和content均可自定义）",
        "inputSchema": {
            "type": "object",
            "properties": {
                "title": {"type": "string"},
