...
async def report(body: ReportBody, req: Request):
    ...
    if auth != f"Bearer {AUTH_TOKEN}": raise HTTPException(401, "Unauthorized")
...
def _get_summary():
    ...
    for r in rows:
        app, ev, ts = r
        if ev == "open": opens[app] = datetime.fromisoformat(ts)
    ...
    for app, open_time in opens.items():
        gap = int((datetime.utcnow() - open_time).total_seconds())
        sessions[app] = sessions.get(app, 0) + gap

    last_active = recent[0][2] if recent else None

    return {"recent_apps": [r[0] for r in recent], "sessions": sessions, "last_active": last_active}
...
def check_on_wife(limit=10):
    ...
    if last_active:
        try:
            ...
            dt_cst = dt + CST
            lines.append(f"🕐 最近活跃: {dt_cst.strftime('%Y-%m-%d %H:%M:%S')}")
        except Exception:
            lines.append(f"🕐 最近活跃: {last_active}")
    else:
        lines.append("🕐 最近活跃: 暂无记录")

    lines.append("=" * 32)
    lines.append(f"📱 最近打开: {', '.join(apps)}" if apps else "📱 最近打开: 暂无记录")
...
# 🔥 方案二核心改动：默认 title 为空，AI 必须自定义标题
def bark_alert(title="", content=""):
    if not content: return "内容不能为空"
    if not BARK_KEY: return "BARK_KEY未配置"
    # 如果 AI 没给 title，用内容前20字当标题
    display_title = title if title else (content[:20] + "..." if len(content) > 20 else content)
    url = f"https://api.day.app/{BARK_KEY}/{display_title}/{content}"
    try:
...
TOOLS = [
    ...
    # 🔥 描述改通用，不绑定查岗
    {"name": "bark_alert", "description": "给手机发Bark推送弹窗（通用工具，不限于查岗场景，title和content均可自定义）", "inputSchema": {"type": "object", "properties": {"title": {"type": "string"}, "content": {"type": "string"}}, "required": ["content"]}}
]
